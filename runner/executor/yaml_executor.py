...
示例:
  ...
  %(prog)s tests/yamls/*.yaml --continue-on-error --summary output.json
    ...
    parser.add_argument(
        "files", nargs="+", help="要执行的 YAML 文件或模式（如 tests/yamls/*.yaml）"
    )
...
class YamlTestRunner:
    ...
    def __init__(
        self, yaml_config: Dict[str, Any], args: Optional[argparse.Namespace] = None
    ):
    ...
    for i, yaml_file in enumerate(yaml_files, 1):
        ...
        if not os.path.exists(yaml_file):
            ...
            if not args.continue_on_error:
                break
            all_results.append(
                {"file": yaml_file, "success": False, "error": "文件不存在"}
            )
            continue
        ...
        try:
            ...
            if not yaml_config:
                ...
                if not args.continue_on_error:
                    break
                all_results.append(
                    {"file": yaml_file, "success": False, "error": "YAML 文件为空"}
                )
                continue
            ...
            all_results.append({"file": yaml_file, "success": False, "error": str(e)})
