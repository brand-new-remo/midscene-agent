...
async def simulate_nodejs_deduplication(operations: List[Dict[str, Any]], time_window: int = 5000) -> Dict[str, Any]:
    """模拟 Node.js 去重中间件"""
    cache = {}
    executed = []
    skipped = []
...
async def simulate_langgraph_memory_saver(operations: List[Dict[str, Any]]) -> Dict[str, Any]:
...
async def test_basic_usage_scenario():
    ...
    operations = [
        ...
        {"action": "find_element", "params": {"description": "JavaScript API 参考菜单项"}},
        {"action": "find_element", "params": {"description": "JavaScript API 参考菜单项"}},  # 重复
        {"action": "find_element", "params": {"description": "JavaScript API 参考菜单项"}},  # 重复
        {"action": "aiQuery", "params": {"query": "查找菜单中的JavaScript API参考"}},
        {"action": "aiQuery", "params": {"query": "查找菜单中的JavaScript API参考"}},  # 重复
    ...
    print(f"   平均每操作: {saver_result['total_messages'] / len(operations):.1f} 条消息")

    # 显示线程状态
    thread_state = list(saver_result['thread_states'].values())[0]
    print(f"   线程状态: {thread_state['task_count']} 个任务, {thread_state['message_count']} 条消息")
    ...
    print(f"   状态持久化: {'✅' if final_message_count > 0 else '❌'} ({final_message_count} 条消息)")
    print(f"   历史上下文: {'✅' if context_preserved else '❌'} ({memory_result['total_records']} 条记录)")
...
async def test_multiple_threads_scenario():
    ...
    for thread_name, operations in test_threads.items():
        ...
        # 去重
        dedup_result = await simulate_nodejs_deduplication(operations)
        print(f"   执行: {len(dedup_result['executed'])}, 跳过: {len(dedup_result['skipped'])}")

        # 记忆
        memory_result = await simulate_python_memory([
            {**op, "result": {"success": True}, "url": op.get("params", {}).get("url", "unknown")}
...
async def test_error_recovery_scenario():
    ...
    operations_with_errors = [
        {"action": "navigate", "params": {"url": "https://example.com"}},
        {"action": "click", "params": {"element": "正常按钮"}, "result": {"success": True}},
        {"action": "click", "params": {"element": "错误按钮"}, "result": {"success": False, "error": "元素未找到"}},
        {"action": "click", "params": {"element": "错误按钮"}, "result": {"success": False, "error": "元素未找到"}},  # 重复错误
        {"action": "find_element", "params": {"description": "替代按钮"}, "result": {"success": True}},
        {"action": "click", "params": {"element": "替代按钮"}, "result": {"success": True}},
    ]
    ...
    for record in memory_result['records']:
        if not record['success']:
            print(f"     ❌ {record['action']}: {record['result'].get('error', 'Unknown error')}")
...
async def main():
    ...
    try:
        ...
        print(f"   ✅ 错误恢复: {results['error_recovery']['success_rate']:.1f}% 成功率")
